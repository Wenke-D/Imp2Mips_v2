...
(** Frame Pointer, bottom of the stack  *)
let fp : register = "$fp"
...
(** Jump to address of label z, save return address to $ra *)
let jal (z : label) = ins "jal %s" z

let jalr r = ins "jalr %s" r

(** Jump to Register: jump to the value of $r as address *)
let jr r = ins "jr %s" r

(** Load value to register r: $r = i  *)
let li r i = ins "li %s, %i" r i

(** Load Address: load address of label [a] into register x  *)
let la x (a : label) = ins "la %s, %s" x a

(** Load byte unsigned, $x = i + $r *)
let lbu x i r = ins "lbu %s, %i(%s)" x i r

(** Load a word to register x, the word address is calculated by i + value of register r *)
let lw x i r = ins "lw %s, %i(%s)" x i r

(** Store Word: memory[i+$t] = $x  *)
...
(** Perform system call based on value in $v0  *)
